...
<?php if ( bp_is_group_admin_screen( 'edit-details' ) ) : ?>
	...
	<p>
		<label for="group-notify-members">
			<input type="checkbox" name="group-notify-members" value="1" /> <?php _e( 'Notify group members of these changes via email', 'thrive' ); ?>
		</label>
	</p>
...
<?php if ( bp_is_group_admin_screen( 'group-settings' ) ) : ?>
	...
	<?php if ( bp_is_active( 'forums' ) ) : ?>
		...
		<?php if ( bp_forums_is_installed_correctly() ) : ?>
			...
			<div class="checkbox">
				<label><input type="checkbox" name="group-show-forum" id="group-show-forum" value="1"<?php bp_group_show_forum_setting(); ?> /> <?php _e( 'Enable discussion forum', 'thrive' ); ?></label>
			</div>
...
<?php if ( bp_is_group_admin_screen( 'group-avatar' ) ) : ?>
	...
	<?php if ( 'upload-image' == bp_get_avatar_admin_step() ) : ?>
			...
			<?php if ( bp_get_group_has_avatar() ) : ?>
				...
				<?php bp_button( array( 'id' => 'delete_group_avatar', 'component' => 'groups', 'wrapper_id' => 'delete-group-avatar-button', 'link_class' => 'edit', 'link_href' => bp_get_group_avatar_delete_link(), 'link_title' => __( 'Delete Group Profile Photo', 'thrive' ), 'link_text' => __( 'Delete Group Profile Photo', 'thrive' ) ) ); ?>
...
<?php if ( bp_is_group_admin_screen( 'manage-members' ) ) : ?>
	...
	<div class="bp-widget">
		<h4><?php _e( 'Administrators', 'thrive' ); ?></h4>
		...
		<ul id="members-list" class="item-list single-line">
			...
			<li>
				<div class="row">
					<div class="col-sm-2">
						<?php echo bp_core_fetch_avatar( array( 'item_id' => bp_get_member_user_id(), 'type' => 'full', 'width' => 150, 'height' => 150, 'alt' => sprintf( __( 'Profile picture of %s', 'thrive' ), bp_get_member_name() ) ) ); ?>
					</div>
					<div class="col-sm-10">
						...
						<?php if ( count( bp_group_admin_ids( false, 'array' ) ) > 1 ) : ?>
							<span class="small">
								<a class="confirm admin-demote-to-member" href="<?php bp_group_member_demote_link( bp_get_member_user_id() ); ?>">
									<?php _e( 'Demote to Member', 'thrive' ); ?>
								</a>
							</span>
						<?php endif; ?>
	...
	<?php if ( bp_group_has_moderators() ) : ?>
		<div class="bp-widget">
			<h4><?php _e( 'Moderators', 'thrive' ); ?></h4>
	...
	<div class="bp-widget">
				...
				<?php while ( bp_group_members() ) : bp_group_the_member(); ?>
					...
					<li class="<?php bp_group_member_css_class(); ?>">
						<div class="row">
							...
							<?php if ( bp_get_group_member_is_banned() ) _e( '(banned)', 'thrive' ); ?>
							...
							<?php else : ?>
								...
								<a href="<?php bp_group_member_ban_link(); ?>" class="confirm member-ban" title="<?php esc_attr_e( 'Kick and ban this member', 'thrive' ); ?>"><?php _e( 'Kick &amp; Ban', 'thrive' ); ?></a>
								<a href="<?php bp_group_member_promote_mod_link(); ?>" class="confirm member-promote-to-mod" title="<?php esc_attr_e( 'Promote to Mod', 'thrive' ); ?>"><?php _e( 'Promote to Mod', 'thrive' ); ?></a>
								<a href="<?php bp_group_member_promote_admin_link(); ?>" class="confirm member-promote-to-admin" title="<?php esc_attr_e( 'Promote to Admin', 'thrive' ); ?>"><?php _e( 'Promote to Admin', 'thrive' ); ?></a>

							<?php endif; ?>

								<a href="<?php bp_group_member_remove_link(); ?>" class="confirm" title="<?php esc_attr_e( 'Remove this member', 'thrive' ); ?>"><?php _e( 'Remove from group', 'thrive' ); ?></a>
		...
		<?php else: ?>
			...
			<div id="message" class="info">
				<p><?php _e( 'This group has no members.', 'thrive' ); ?></p>
			</div>
