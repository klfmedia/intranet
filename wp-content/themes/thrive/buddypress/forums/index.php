<div id="buddypress">
	...
	<form action="" method="post" id="forums-directory-form" class="dir-form">
		...
		<div class="item-list-tabs" role="navigation">
			<ul>
				<li class="selected" id="forums-all"><a href="<?php echo trailingslashit( bp_get_root_domain() . '/' . bp_get_forums_root_slug() ); ?>"><?php printf( __( 'All Topics <span>%s</span>', 'thrive' ), bp_get_forum_topic_count() ); ?></a></li>

				<?php if ( is_user_logged_in() && bp_get_forum_topic_count_for_user( bp_loggedin_user_id() ) ) : ?>

					<li id="forums-personal"><a href="<?php echo trailingslashit( bp_loggedin_user_domain() . bp_get_forums_slug() . '/topics' ); ?>"><?php printf( __( 'My Topics <span>%s</span>', 'thrive' ), bp_get_forum_topic_count_for_user( bp_loggedin_user_id() ) ); ?></a></li>
		...
		<div class="item-list-tabs" id="subnav" role="navigation">
			<ul>
				...
				<li id="forums-order-select" class="last filter">
					...
					<label for="forums-order-by"><?php _e( 'Order By:', 'thrive' ); ?></label>
					<select id="forums-order-by">
						<option value="active"><?php _e( 'Last Active', 'thrive' ); ?></option>
						<option value="popular"><?php _e( 'Most Posts', 'thrive' ); ?></option>
						<option value="unreplied"><?php _e( 'Unreplied', 'thrive' ); ?></option>
		...
		<?php if ( is_user_logged_in() ) : ?>
			...
			<?php if ( bp_is_active( 'groups' ) && bp_has_groups( 'user_id=' . bp_loggedin_user_id() . '&type=alphabetical&max=100&per_page=100' ) ) : ?>
				...
				<form action="" method="post" id="forum-topic-form" class="standard-form">
					...
					<a name="post-new"></a>
					<h5><?php _e( 'Create New Topic:', 'thrive' ); ?></h5>
					...
					<label><?php _e( 'Tags (comma separated):', 'thrive' ); ?></label>
					<input type="text" name="topic_tags" id="topic_tags" value="" />

					<label><?php _e( 'Post In Group Forum:', 'thrive' ); ?></label>
					<select id="topic_group_id" name="topic_group_id">

						<option value=""><?php /* translators: no option picked in select box */ _e( '----', 'thrive' ); ?></option>
					...
					<div class="submit">
						<input type="submit" name="submit_topic" id="submit" value="<?php esc_attr_e( 'Post Topic', 'thrive' ); ?>" />
						<input type="button" name="submit_topic_cancel" id="submit_topic_cancel" value="<?php esc_attr_e( 'Cancel', 'thrive' ); ?>" />
					</div>
				...
				<div id="message" class="info">
					...
					<p>
						...
						printf(
							...
							__( "You are not a member of any groups so you 
								 don't have any group forums you can post in. To start posting, 
								 first find a group that matches the topic subject you'd like to start. 
								 If this group does not exist, why not <a href='%s'>create a new group</a>? 
								 Once you have joined or created the group you can post your topic in that 
								 group's forum.", 'thrive' 
							),
